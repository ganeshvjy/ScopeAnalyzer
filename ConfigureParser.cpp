...
Parser* ConfigParseToConsole::Build()
{
  try
  {
	...
	pConstructDefinition = new ConstructDefinition;
	pPushConstruct = new PushConstruct(pRepo);
	pConstructDefinition->addAction(pPushConstruct);
	pParser->addRule(pConstructDefinition);
